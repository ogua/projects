      ...
      <div class="container-fluid">
			<div class="panel panel-info">
				...
				<div class="panel panel-body">
					 <?php
            include('db.php');
			$sqlAll = mysqli_query($conn,"SELECT count(*) as numofrows FROM `fertilizer`");
			$row = mysqli_fetch_assoc($sqlAll);
			$allrow = $row['numofrows'];
			//get rows query
			$query = mysqli_query($conn, "SELECT * FROM `fertilizer`");
			//number of rows
			$rowCount = mysqli_num_rows($query);
			if($rowCount > 0){ 
			echo'
			<div class="table-responsive">
				<button class="btn btn-default">Total Fertilizer&nbsp;<span style="font-size:1.0em;"class="badge">'.$allrow.'</span></button>
				<a href="fertilizer.php" class="btn btn-default">Record New Fertilizer&nbsp;<i class="fa fa-plus"></i></a>
...
<script>
		$('document').ready(function(){
			...
			$(".deluser").click(function(e){
				...
				//alert(cid);
				if(confirm("Are you sure you want to delect this Fetilizer")){
					$.ajax({
						url: 'action.php',
						type: 'POST',
						data: {DelFertizer: 1, cid: cid},
						success: function(data){
							if(data.match("success")){
								alert("User Delected Successfully!");
								window.location.href="vfertilizer.php"
							}else{
								alert("You Dont Have the right Permission to Delect this Item");
							}
						}
					});
				}
