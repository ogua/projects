...
function fetch_data()  
 {  
	  include('db.php');
      $output = '';  
      $sqlAll = mysqli_query($conn,"SELECT count(*) as numofrows FROM `payments`  WHERE username = '".$_POST['nameofin']."' and `dates` BETWEEN '".$_SESSION['frm']."' AND '".$_SESSION['to']."' ");
			$row = mysqli_fetch_assoc($sqlAll);
			$allrow = $row['numofrows'];
			//get rows query
			$sql = "SELECT * FROM `payments` WHERE username = '".$_POST['nameofin']."' and `dates` BETWEEN '".$_SESSION['frm']."' AND '".$_SESSION['to']."' ";
		...
		';
			...
			$output.='
			 ...
			 return $output;
				...
				header("location:inditransacReport.php?err=No record Found, Please Choose a Different Date");
			}
 }  
if(isset($_POST["generate_pdf"]))  
 {  
      ...
      $obj_pdf->AddPage();  
      $content = '';
	  $content .= '<h2>Report Generated for Transactions</h2>';	  
      $content .= fetch_data();  
...
<head>
  ...
  <body>
			<div class="container-fluid" style="margin-top:30px;">
				<h2>Report Generated for <?php echo $_POST['nameofin']?></h2>
					<a href="inditransacReport.php" class="btn btn-success">&larr; Back </a>
				 <?php  
                     echo fetch_data();  
                     ?>  
			</div>
