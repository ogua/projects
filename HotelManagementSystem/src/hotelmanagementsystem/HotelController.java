...
public class HotelController implements Initializable {
    ...
    private void Update1(ActionEvent event) {
         ...
         String sql = "UPDATE `room` SET `SingleRoom` = '"+MU1.getText()+"'";
        try {
    ...
    private void Update2(ActionEvent event) {
      conn = MysqliConnect.Dbconnect();
      String sql = "UPDATE `room` SET `TwoRoom` = '"+MU2.getText()+"'";
      try {
            pr = conn.prepareStatement(sql);
            pr.executeUpdate();
    ...
    private void Update3(ActionEvent event) {
         conn = MysqliConnect.Dbconnect();
         String sql = "UPDATE `room` SET `ThreeBedRoom` = '"+MU3.getText()+"'";
        try {
            pr = conn.prepareStatement(sql);
            pr.executeUpdate();
    ...
    private void getMealPrice(){
     conn = MysqliConnect.Dbconnect();
     String sql = "SELECT `SingleRoom` FROM room";
        try {
            pr = conn.prepareStatement(sql);
            rs = pr.executeQuery();
            if(rs.next()){
              String price1 = rs.getString("SingleRoom");
    ...
    conn = MysqliConnect.Dbconnect();
     String sql = "SELECT `TwoRoom` FROM room";
      try {
            pr = conn.prepareStatement(sql);
            rs = pr.executeQuery();
            if(rs.next()){
              String price1 = rs.getString("TwoRoom");
              int price11 = Integer.parseInt(price1);
    ...
    conn = MysqliConnect.Dbconnect();
     String sql = "SELECT `ThreeBedRoom` FROM room";
      try {
            pr = conn.prepareStatement(sql);
            rs = pr.executeQuery();
            if(rs.next()){
              String price1 = rs.getString("ThreeBedRoom");
              int price11 = Integer.parseInt(price1);
