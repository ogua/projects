...
<div class="wrapper">
  ...
  <div class="content-wrapper">
        ...
        <div class="col-md-12">
          ...
          <div class="box box-primary">
			...
			<i class="glyphicon glyphicon-user"></i>
              <h3 class="box-title">Transactions<span><small> &nbsp; > View / Delete Transactions</small></span></h3>
	...
	<div id="displayerrormsghere">
            ...
            include('db.php');
			if(@isset($_GET['id'])){
				$sqlAll = mysqli_query($conn,"SELECT count(*) as numofrows FROM `payments`");
		...
		';
				...
				<td><ul class="list-inline">
						<li><a href="admindelet.php?delt='.$row['id'].'" class="btn btn-danger"><span class="fa fa-trash"></span></a></li>
						<li><a href="admindelet.php?editt='.$row['id'].'" class="btn btn-success"><span class="fa fa-edit"></span></a></td></li>
            ...
            <div id="displayerrormsghere"></div>
					...
					<?php
						if(@isset($_GET['edit'])){
							$upid = $_GET['edit'];
							$supade = "SELECT * FROM `payments` WHERE `id` = $upid";
							$squery = mysqli_query($conn,$supade);
							$rows = mysqli_fetch_array($squery);
							echo'
								<div class="row">
						<div class="col-md-3">
							<img src="images/'.$rows['image'].'" width="200" height="200">
						</div>
						<div class="col-md-8">
							<form method="Post" action="">
								<div class="form-group">
									<label>Fullname</label>
									<input type="text" class="form-control" value="'.$rows['username'].'" name="fuuname" id="fuuname"  disabled>									
								</div>
								<div class="form-group">
									<label>Phone Number</label>
									<input type="text" class="form-control" value="'.$rows['phonenume'].'" name="fnnumb" id="fnnumb"  disabled>								</div>
								<div class="form-group">
									<label>Email</label>
									<input type="text" class="form-control" value="'.$rows['email'].'" name="emails" id="emails" disabled>									
								</div>
								<div class="form-group">
									<label>Slot Position</label>
									<input type="text" class="form-control" value="'.$rows['slotename'].'" name="slotpo" id="slotpo" disabled>
								</div>
								<div class="form-group">
									<label>Slot id</label>
									<input type="text" class="form-control" value="'.$rows['assignslot'].'" name="slotid" id="slotid" disabled>									
								</div>
								<div class="form-group">
									<label>Vehicle</label>
									<input type="text" class="form-control" value="'.$rows['vehicle'].'" name="slotpo" id="slotpo" disabled>
								</div>
								<div class="form-group">
									<label>Vehicle Plate</label>
									<input type="text" class="form-control" value="'.$rows['platenumber'].'" name="slotid" id="slotid" disabled>									
								</div>
								<div class="form-group">
									<label>Parking Days</label>
									<input type="text" class="form-control" value="'.$rows['parkdays'].'" name="slotpo" id="slotpo" disabled>
								</div>
								<div class="form-group">
									<label>Amount to Pay</label>
									<input type="text" class="form-control" value="'.$rows['amountpaid'].'" name="slotpo" id="slotpo" disabled>
								</div>
								<div class="form-group">
									<label>Status</label>
									<input type="text" class="form-control" value="'.$rows['status'].'" name="slotpo" id="slotpo" disabled>
								</div>
								<div class="form-group">
									<label>Update Payment</label>
									<select name="updatepay" id="updatepay"class="form-control">
...
<script>
	$("document").ready(function(){
		...
		$("#updatepay").change(function(e){
			e.preventDefault();	
			var valeus = $("#updatepay").val();
			var id = '<?php echo $rows['id'];?>';
			$.ajax({
				url: 'admindelet.php',
				method: 'POST',
				data: {updatest: 1, valeus: valeus, id: id},
