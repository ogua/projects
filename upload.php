	...
	if(is_array($_FILES)){
		include_once("db.php");
			...
			if(move_uploaded_file($tmpimage,$source.$filename)){
					$sql3 = "INSERT INTO `DrugInfo`(`userid`, `images`, `manufacturername`, `drugname`, `brand`, `manufacdate`, `Expdate`, `manNumber`, `Distributor`, `serialNumber`, `dates`) VALUES 
					('$userid','$filename','$mname','$dname','$brand','$muDate','$Exdate','$mauNumber','$comdisrt','$SerialNumber',CURRENT_DATE)";
