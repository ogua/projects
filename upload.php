	...
	if(is_array($_FILES)){
		include_once("db.php");
			...
			$filename = round(microtime(true)).mt_rand().'.'.$ext;
				if($image ==""){
					$sql3 = "INSERT INTO `users`(dates,`fullname`, `username`, `role`, `password`, `email`, `phone`, `image`) 
					VALUES (CURRENT_DATE,'$fname','$uname','Customer','$passv','$email','$phone','co.png')";
					...
					VALUES ('$fname','$uname','Customer','$passv','$email','$phone','$filename')";
					$query3 = mysqli_query($conn,$sql3);
					if($query3){
						echo"Successfully";
					}else{
