	  	  ...
	  	  <hr style="border:2px solid #ccc;">
				<ul class="list-unstyled">
					<li><a href="Bookprg.php" class="btn btn-success">New Booking</a></li>
						  	  <hr style="border:2px solid #ccc;">
					<li><a href="accountall.php" class="btn btn-success">view all Booking</a></li>
				</ul>
			</div>
			<div class="col-md-10">
	        <h3 style="padding:20px;font-size:60px;color:white;text-align:center;font-family:Tahoma;">Resent Booking</h3>
				<?php
            include('db.php');
			$sqlAll = mysqli_query($conn,"SELECT count(*) as numofrows FROM `payments` WHERE `username` = '".$_SESSION['name']."' AND dates = CURRENT_DATE");
			$row = mysqli_fetch_assoc($sqlAll);
			$allrow = $row['numofrows'];
			//get rows query
			$query = mysqli_query($conn, "SELECT * FROM `payments` WHERE `username` = '".$_SESSION['name']."' AND dates = CURRENT_DATE ");
			//number of rows
