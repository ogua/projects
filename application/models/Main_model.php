...
class Main_model extends MY_Model
{
    ...
    public function users_list()
    {
        $this->db->select("uu.USER_ID, uu.logged_in, uu.CREATED_DATE,
                    uu.USER_NAME,uu.USER_ID, ug.GROUP_NAME,ug.GROUP_ID, uu.IS_AC
TIVE")->from("usr_group  as ug,
...
INNER JOIN stock ON stock.item_id = item.item_id AND stock.category_id = categor
y.category_id
...
");
    ...
    public function invoice($id)
    {
        $query = $this->db->query("SELECT * FROM qoutation
                    JOIN qoutation_detail ON qoutation_detail.qoutation_id=qouta
tion.qoutaion_id
...
");
    ...
    public function getAllissue_twodate($idfiletype, $date, $todate)
    {
        ...
        $sql = $this->db->select("issue.idissue, issue.datetime, issue.dairyno,
issue.barcode, issue.subject,
 department.department, volume.volume, filestatus.filestatus, filetype.filetype,
 flag.flag")->from("issue,department,filestatus,filetype,flag,source,volume")
...
department.department, volume.volume, filestatus.filestatus, filetype.filetype,
flag.flag, receive.idreceive,receive.received
FROM
    track
    INNER JOIN issue
    ...
    public function countAll_pending($idfiletype)
    {
        ...
        $sql = "SELECT i.datetime as datetime, i.barcode as barcode, i.subject as subject, f.filetype as filetype, g.flag as flag, d.department as department, s.section as section, r.idissue as idissue, r.idreceive as idreceive, r.received
as received, i.scan as scan, t.filestatus as filestatus
                  FROM receive as r, issue as i, filetype as f, flag as g, depar
tment as d, section as s, filestatus as t
 ...
 track.datetime,track.fromdept,track.fromsection,track.remarks,track.todept ,
  track.tosection,flag.flag,department.department,filestatus.filestatus,filetype
.filetype,source.source,volume.volume,
...
AND receive.status = 1
    ...
    {
        $sql = "SELECT i.fileno as fileno,t.idfilestatus, i.barcode as barcode,
i.dairyno as dairyno, i.subject as subject, f.filetype as filetype, g.flag as fl
ag, d.department as department,
 r.idissue as idissue, r.idreceive as idreceive, r.received as received, i.scan
as scan, t.filestatus as filestatus
                  FROM `receive` as r, `issue` as i, `filetype` as f, flag as g,
 department as d, filestatus as t
                  WHERE r.idissue = i.idissue
                  AND i.idfiletype = f.idfiletype
                  AND i.idflag = g.idflag
                  AND i.fromdept = d.iddepartment
