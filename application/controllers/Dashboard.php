...
class Dashboard extends MY_Controller
{
    ...
    public function index()
    {
      ...
      //  if ($this->session->userdata('group_id') == 1) {
            ...
            $data['datas'] = $this->db->query("SELECT fm.returned_status,fm.file_id,fm.file_m_id,fm.`date_collected`,fm.`file_no`,fm.`name_of_file`,
fm.`number_of_letters`,fm.`remarks`,fm.`status` FROM file_movement AS fm WHERE fm.status = 1 AND fm.returned_status = 'Unreturned'")->result_array();
            $data['issueds'] = $this->db->query("SELECT 
  m.*,d.`filetype`,fs.`filestatus`,f.`flag`,dp.`department`,ml.`date_of_sending`,
...
WHERE d.`idfiletype` = m.`doc_id` 
  ...
  AND f.`idflag` = m.`flag_id` 
  AND ml.main_id = m.`main_id`  
  AND m.`status` IN (9, 3) AND ml.r_status = 'forwarded' AND ml.date_of_received = '$todate'")->row_array();
...
FROM
  ...
  WHERE  om.`status` IN(1,0) 
   AND MONTH(om.`dispatch_date`) = MONTH('$todate')")->row_array();
        $data['outgoingYear'] = $this->db->query("SELECT 
  COUNT(1) AS outgoing
FROM
  outgoing_mails AS om 
  WHERE  om.`status` IN(1,0) 
   AND YEAR(om.`dispatch_date`) = YEAR('$todate')")->row_array();
...
WHERE d.`idfiletype` = m.`doc_id` 
  AND fs.`idfilestatus` = m.`status`
 AND f.`idflag` = m.`flag_id`
  AND m.`date_issue` = '$todate'
  AND m.`user_id` = u.`USER_ID`
  ORDER BY m.main_id DESC")->row_array();


        $data['receivedYear'] = $this->db->query("SELECT 
  COUNT(1) AS received 
FROM
  main AS m,
  filetype AS d,
...
WHERE d.`idfiletype` = m.`doc_id` 
  AND fs.`idfilestatus` = m.`status`
 AND f.`idflag` = m.`flag_id`
  AND YEAR(m.`date_issue`) = YEAR('$todate')
  AND m.`user_id` = u.`USER_ID`
  ORDER BY m.main_id DESC")->row_array();
...
COUNT(1) dispatch
FROM
  outgoing_mails AS om WHERE om.`status` IN(1,0) AND om.dispatch_date = '$tdate'")->row_array();

        $data['dispatchedYear'] = $this->db->query("SELECT 
COUNT(1) dispatch
FROM
  outgoing_mails AS om WHERE om.`status` IN(1,0) AND YEAR(om.dispatch_date) = YEAR('$tdate')")->row_array();

            $data['department'] = $this->db->query("SELECT COUNT(1) as department FROM department")->row_array();

            $data['users'] = $this->db->query("SELECT COUNT(1) as users FROM usr_user ")->row_array();
...
FROM
  outgoing_mails AS om 
WHERE om.`status` IN (0, 1) AND MONTH(om.dispatch_date) = '$today'")->result_array();
       // echo $this->db->last_query();
        $this->header();
...
WHERE d.`idfiletype` = m.`doc_id` 
 ...
 AND f.`idflag` = m.`flag_id`
  ...
  ORDER BY m.main_id DESC")->result_array();
    ...
    public function yearlyReceived()
    {
        $today = date('Y-m-d');
        /*$data['issueds'] = $this->db->query("SELECT
   m.*,d.`doc_type`,fs.`filestatus`,f.`flag`
...
WHERE d.`idfiletype` = m.`doc_id` 
  AND fs.`idfilestatus` = m.`status`
 AND f.`idflag` = m.`flag_id`
 AND YEAR(m.date_received) = YEAR('$today')
  AND m.`user_id` = u.`USER_ID` order by m.main_id DESC")->result_array();
        //echo $this->db->last_query();
        $this->header();
        $this->load->view('reports/yearlyReceived', $data);
        $this->footer();
  ...
  om.`dispatch_date`,om.om_id,om.status,om.registry_number,om.scan_copy,om.remarks,
  om.`date_of_letter`,om.`reference_number`,
  om.`subject`,om.`cell_no`,om.scan_copy_printed
FROM
  outgoing_mails AS om WHERE om.`status` IN(1,0) AND om.dispatch_date = '$today'")->result_array();
        $this->header();
    ...
    public function yearlyDispatched()
    {
        $today = date('Y-m-d');
        $data['mails'] = $this->db->query("SELECT 
  om.`dispatch_date`,om.om_id,om.status,om.registry_number,om.scan_copy,om.remarks,
  ...
  om.`subject`,om.`cell_no`,om.scan_copy_printed
FROM
  outgoing_mails AS om WHERE om.`status` IN(1,0) AND YEAR(om.dispatch_date) = YEAR('$today')")->result_array();
        $this->header();
        $this->load->view('reports/yearlyDispatched', $data);
        $this->footer();
...
WHERE d.`idfiletype` = m.`doc_id` 
  ...
  AND f.`idflag` = m.`flag_id` 
  AND ml.main_id = m.`main_id`  
  AND m.`status` IN (9, 3) AND ml.r_status = 'forwarded' AND ml.date_of_received = '$todate'")->result_array();
 $this->header();
