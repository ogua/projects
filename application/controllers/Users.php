...
class users extends MY_Controller
{
    ...
    public function loginauthen()
    {
        ...
        if ($login) {
            ...
            $emp = $this->db->query("SELECT e.`EMP_NAME` as empname,e.EMP_PIC FROM employee_profile AS e
WHERE e.`EMP_ID` = $id_emp")->row_array();
    ...
    public function add_user()
    {
        ...
        $data['employeelist'] = $this->General->fetch_records("employee");
        $data['grouplist'] = $this->General->fetch_records("usr_group");
        //Get user's list........................................................
        $data['userlist'] = $this->General->fetch_CoustomQuery("SELECT 
  emp.`empname`,emp.`idemployee`,uu.`USER_NAME`,uu.IS_ACTIVE,uu.`USER_ID`,ug.`GROUP_ID`,ug.`GROUP_NAME` 
FROM
  employee AS emp,
  usr_group AS ug,
  usr_user AS uu
WHERE uu.GROUP_ID = ug.GROUP_ID 
  AND uu.idemployee = emp.idemployee ");
...
//echo $this->db->last_query();exit;
    ...
    {
		$per = $this->getdeleteper();
		if($per == 1){
			
		}else{
			$data = array(
				'msg'=>'Delete Access Denied',
				'errortype'=>'alert-success'
			);
    ...
    public function registration()
    {
		...
		if($per == 1){
			...
			$data = array(
				'msg'=>'User Creation Access Denied',
				'errortype'=>'alert-success'
			);
			$this->session->set_userdata($data);
			 redirect(base_url() . "index.php/users/add_user");
