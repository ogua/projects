...
$pdf->cell(24,8,"Contact",1,0);
$pdf->cell(55,8,"Email",1,0);
$pdf->cell(30,8,"Registred",1,1);

$pdf->SetFont('Times','',13);
$sql = "SELECT * FROM `patiencedetails` where dates between '".$_POST['frm']."' AND '".$_POST['to']."'";
$query = mysqli_query($conn,$sql);
...
if($result > 0){
	while($row = mysqli_fetch_array($query)){
		...
		$pdf->cell(20,7,$cfirst,1,0);
		$ladd = strtolower($row['address']);
		$addfirst = ucwords($ladd);
		$pdf->cell(45,7,$addfirst,1,0);
		$pdf->cell(24,7,$row['contactno'],1,0);
		$cadd = strtolower($row['email']);
		$catfirst = ucwords($cadd);
		$pdf->cell(55,7,$catfirst,1,0);
			...
			<div class="table-responsive" >	
						...
						<thead class="bg-danger">
							<tr>
								...
								<th style="color:red;"><b>Registred</b></th>
							</tr>
						</thead>
						<tbody>';
			while($row = mysqli_fetch_array($query)){
