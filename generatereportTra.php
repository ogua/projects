...
function fetch_data()  
 {  
	  include('db.php');
      $output = '';  
      $sqlAll = mysqli_query($conn,"SELECT count(*) as numofrows FROM `payments`  WHERE `dates` BETWEEN '".$_SESSION['frm']."' AND '".$_SESSION['to']."' ");
			$row = mysqli_fetch_assoc($sqlAll);
			$allrow = $row['numofrows'];
			//get rows query
			$sql = "SELECT * FROM `payments` WHERE `dates` BETWEEN '".$_SESSION['frm']."' AND '".$_SESSION['to']."' ";
		...
		';
			...
			$output.='
			 ...
			 return $output;
				...
				header("location:transacReport.php?err=No record Please Choose a Different Date");
			}
 }  
...
<head>
  ...
  <body>
			<div class="container-fluid" style="margin-top:30px;">
				<h2>Report Generated for Transactions</h2>
					<a href="transacReport.php" class="btn btn-success">&larr; Back </a>
				 <?php  
                     echo fetch_data();  
                     ?>  
			</div>
