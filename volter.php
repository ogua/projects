    ...
    <section class="content">
      ...
      <div class="container-fluid">
			<div class="panel panel-info">
				<div class="panel panel-heading">Start Voting</div>
				<div class="panel panel-body" id="updatevoting">
				
				<?php
               include('db.php');
			   //get rows query
			   $query = mysqli_query($conn, "SELECT * FROM `positions` order by id desc LIMIT 1");
			  //number of rows
...
<script>
		$('document').ready(function(){
			...
			$("#startvoting").click(function(e){
				e.preventDefault();
				if(confirm("Are you sure you want to start Voting")){
					window.location.href="volter.php";
					/*$.ajax({
						url: 'action.php',
						type: 'POST',
			...
			$(document).on("click",".radioinput",function(e){
				...
				var cid = $(this).attr("cid");
				var name = $(this).val();
				var volterid  = '<?php echo $_SESSION['id']; ?>';
				var position = $("#position_"+cid).val();
				if(confirm("Do you want to vote for "+ name)){
					$.ajax({
						url: 'action.php',
						type: 'POST',
						data: {Vote: 1, cid: cid, name: name, volterid: volterid, position: position},
						success: function(data){
							$("#position_"+cid).prop("checked",true);
