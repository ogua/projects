...
$pdf->cell(22,8,"Doc ID",1,0);
$pdf->cell(40,8,"Name",1,0);
$pdf->cell(45,8,"Address",1,0);
$pdf->cell(24,8,"Mobile",1,0);
$pdf->cell(35,8,"Category",1,0);
$pdf->cell(30,8,"Added",1,1);

$pdf->SetFont('Times','',13);
$sql = "SELECT * FROM `doctors` where dates between '".$_POST['frm']."' AND '".$_POST['to']."'";
$query = mysqli_query($conn,$sql);
$result = mysqli_num_rows($query);
if($result > 0){
	while($row = mysqli_fetch_array($query)){
		$pdf->cell(22,7,$row['docID'],1,0);
		$lname = strtolower($row['name']);
		$cfirst = ucwords($lname );
		$pdf->cell(40,7,$cfirst,1,0);
		$ladd = strtolower($row['address']);
		$addfirst = ucwords($ladd);
		$pdf->cell(45,7,$addfirst,1,0);
		$pdf->cell(24,7,$row['mobi'],1,0);
		$cadd = strtolower($row['category']);
		$catfirst = ucwords($cadd);
		$pdf->cell(35,7,$catfirst,1,0);
			...
			<div class="table-responsive" >	
						...
						<thead class="bg-danger">
							<tr>
								...
								<th style="color:red;"><b>Recruted</b></th>
							</tr>
						</thead>
						<tbody>';
			while($row = mysqli_fetch_array($query)){
