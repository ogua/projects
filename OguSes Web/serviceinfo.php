                ...
                <div class="content-sing-w3pvt px-lg-5">
                    ...
                    <div class="row about-w3pvt-top mt-2">
                        ...
                        <div class="col-lg-6 about-info">
                            <h4 class="title-hny mb-3">Services Rendered Under <?php echo $row['type'];?></h4>
                            <p><?php echo $row['services'];?></p>


                        </div>
                        <div class="col-lg-6 about-img">
						<h4 class="title-hny mb-3">Projects Developed Under <?php echo $row['type'];?></h4>
                            <div class="row">
								<?php
									$cat = $row['type'];
									$rsql = "SELECT imagesurl,id FROM `projects` WHERE `category` = '$cat' ORDER BY `id` DESC LIMIT 2";
                ...
                <div class="comments my-5 px-lg-5">
                    ...
                    <div class="comments-grids mt-4">
						...
						<?php
							...
							$id =  $_GET['id'];
							if(!empty($id) && is_numeric($id)){
								$sql = "SELECT * FROM `ucomments` WHERE `proid` = '$id' ";
							    $query = mysqli_query($conn,$sql);
								...
								if($result > 0){
									while($rows = mysqli_fetch_array($query)){
										...
										<?php
											$comid = $rows['id'];
										$rsql = "SELECT * FROM `ureply` WHERE `comtid` =  '$comid'";
										$rquery = mysqli_query($conn,$rsql);
			...
			$.ajax({
					...
					success: function(data){
					   if(data.match("emailerror")){
							...
							alert('Your Reply will be displayed Shortly');
