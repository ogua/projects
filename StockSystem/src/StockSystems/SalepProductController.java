 ...
 * @author ogua
 */
public class SalepProductController implements Initializable {
    @FXML
    private VBox main;
    @FXML
   ...
   @FXML
    private void SreachProduct(ActionEvent event) {
    conn = MysqliConnect.dbConnect();
    String sql = "SELECT * FROM `sale` WHERE Productid = ? ORDER BY BillId DESC";
    try{
    pr = conn.prepareStatement(sql);
    pr.setString(1, searchBill.getText());
    rs = pr.executeQuery();
    while(rs.next()){
