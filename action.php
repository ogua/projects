<?php
	...
	if(isset($_POST['getCategory'])){
		...
		if($query){
			...
			while($row = mysqli_fetch_array($query)){
				$catid = $row['catID'];
				echo'
				<li><a href="#" cid="'.$row['catID'].'" class="getCateg">'.$row['CatTitle'].'</a></li>
				';
			}
			echo'</ul>';
		}else{
	...
	if(isset($_POST['getCategory2'])){
		...
		if($query){
			...
			while($row = mysqli_fetch_array($query)){
				$catid = $row['catID'];
				echo'
					<a href="#" cid="'.$row['catID'].'" class="getCateg" >
						  <div class="tab">
							 <ul class="place">								
								<li class="sort">'.$row['CatTitle'].'</li>
								 <li class="by glyphicon glyphicon-arrow-right"></li>
	...
	if(isset($_POST['getCateg'])){
		$id = $_POST['cid'];
		$sql = "SELECT * FROM `products` WHERE `prod_cat` = $id order by rand() limit 0,10";
		$query = mysqli_query($conn,$sql);
		if($query){
			while($row = mysqli_fetch_array($query)){				;
				echo'
				   <a href="single.php?id='.$row['id'].'"><div class="product-grid love-grid">
						<div class="more-product"><span> </span></div>						
						<div class="product-img b-link-stripe b-animate-go  thickbox">
							<img src="images/'.$row['prod_img'].'" class="img-responsive" width="276" height="380" alt=""/>
							<div class="b-wrapper">
							<h4 class="b-animate b-from-left  b-delay03">							
							<button class="btns">view Details</button>
							</h4>
							</div>
						</div></a>						
						<div class="product-info simpleCart_shelfItem">
							<div class="product-info-cust prt_name">
								<h4>'.$row['prod_title'].'</h4>
								<span class="item_price">GH&cent;'.$row['prod_px'].'.00</span>
								<a href="#" cid="'.$row['id'].'" class="glyphicon glyphicon-shopping-cart insercart item_add items" style="text-decoration:none;">ADD</a>
							</div>													
							<div class="clearfix"> </div>
	...
	if(isset($_POST['getProd'])){
		...
		if(isset($_POST['setPages'])){
		    $page = $_POST['id'];
		    $pageStart = ($page * 16) - 16;
		}else{
			$pageStart = 0;
		}
		$sql = "SELECT * FROM `products` order by rand() limit $pageStart,$limit";
		$query = mysqli_query($conn,$sql);
		if($query){
			while($row = mysqli_fetch_array($query)){				;
				echo'
			...
			while($row = mysqli_fetch_array($query)){	
				...
				<a href="single.php?id='.$row['id'].'"><div class="product-grid love-grid">
						...
						<div class="product-info simpleCart_shelfItem">
							...
							<div class="clearfix"> </div>
							<div class="form-group" class="rating">
	...
	if(isset($_POST['getCart'])){
		...
		if(isset($_POST['insercart'])){
			...
			$genid = $_POST['gemnid'];
		if($user == ""){
			/*echo"Please Login or create account to continue shopping";
			exit;*/
			...
			if($userquery){
			   ...
			   if($userresult3 == 1){
				   	echo'Item Has Already Been Added To Cart, Continue Shopping';
			   }else{
				   $usersql3 = "INSERT INTO `cart`(Sellerid,`prod_id`, `generateid`, `prod_title`, `prod_image`, `qty`, `price`, `totalpx`,date)
				VALUES ('$sellid','$id','".$_SESSION['genrateid']."' ,'$prtitle','$prdimage','1','$prdpx','$prdpx',CURRENT_DATE) ";
		...
		if($query){
			...
			if($result3 == 1){
				...
				$sql3 = "INSERT INTO `cart`(`prod_id`, `user_id`, `Sellerid`,`generateid`,`prod_title`, `prod_image`, `qty`, `price`, `totalpx`, date)
				VALUES ('$id','$user','$sellid','".$_SESSION['genrateid']."','$prtitle','$prdimage','1','$prdpx','$prdpx',CURRENT_DATE) ";
			 ...
			 <div class="cart-header'.$x.'">
				 ...
				 <div class="cart-sec">
					   ...
					   <div class="cart-item-info">
							 ...
							 <p class="qty">Qty ::</p>
							 <input type="number" cid="'.$prod_id.'" id="key-'.$prod_id.'" name="quantity" value="1" class="updatetot form-control input-small">
					   </div>
					   <div class="clearfix"></div>
						<div class="delivery">
	...
	if(isset($_POST['updakey'])){
		$id = $_POST['cid'];
		$qty = $_POST['text'] ;
		$slsql = "select * from cart where id = '$id'";
		$querysl = mysqli_query($conn,$slsql);
		$row = mysqli_fetch_array($querysl);
		$px = $row['price'];
		$total = $qty * $px;
		$sql = "UPDATE `cart` SET `qty` = $qty, `totalPx` = $total WHERE `id` = $id";
		$query = mysqli_query($conn,$sql);
		if($query){
			$sltsql = "SELECT * from `cart` WHERE `id` = '$id' ";
			$sltquery = mysqli_query($conn,$sltsql);
			$rows = mysqli_fetch_array($sltquery);
			//json_encode($rows);
			echo $rows['totalpx'].'.00 Qty '.$rows['qty'].'';
		}else{
			echo $id.$qty;
		}
	}
...
Failed to Insert Book Details Try Again Later ';
  ...
  if(isset($_POST['CusOrder'])){
	    $sql = "SELECT * FROM `customerorder` group by ReFid";
		$query = mysqli_query($conn,$sql);
		...
		if($query){
			...
			<ul class="list-inline">
					...
					<li><input type="submit" class="btn btn-danger" name="delorder" id="delorder" value="Delete Order"></li>
					  ...
					  <div class="row">
					  <div class="col-md-2" style="font-size:15px;color:red;">Full name</div>
					  ...
					  <div class="col-md-2" style="font-size:15px;color:red;">Status</div>
			...
			while($row = mysqli_fetch_array($query)){
					...
					<div class="col-md-2">'.$row['Fullname'].'</div>
					  ...
					  <div class="col-md-2">';
						if($row['Status'] == 0){
								echo '<a href="#" class="btn btn-info" style="width:140px;">Processing</a>';
							}elseif($row['Status'] == 1){
								echo '<a href="#" class="btn btn-success" style="width:140px;">Completed</a>';
							}elseif($row['Status'] == 2){
								echo '<a href="#" class="btn btn-default" style="width:140px;">Ready For Pickup</a>';
							}elseif($row['Status'] == 3){
								echo '<a href="#" class="btn btn-danger" style="width:140px;">Cancelled</a>';
							}
					  echo '</div>
					  <div class="col-md-2">
					  <select class="form-control" cid='.$row['ReFid'].' id="UpSta" name="UpSta">
			...
			while($row = mysqli_fetch_array($query)){
					...
					<div class="col-md-1">'.$row['id'].'</div>
					  ...
					  <div class="col-md-2">'.$row['prod_px'].'</div>
					   <div class="col-md-2"><a href="#" cid="'.$row['id'].'" class="deleBUK btn btn-danger" ><span class="glyphicon glyphicon-trash"></span></a>
					   <a href="#" cid="'.$row['id'].'" class="editBUK btn btn-info" ><span class="glyphicon glyphicon-edit"></span></a>
					   </div>
					  </div>					 
					 ';
			}
  ...
  if(isset($_POST['getgenrate'])){
	...
	if(isset($_POST['Acountcreate'])){
		...
		<div class="alert alert-danger"> <a href="#" class="close" data-dismiss="alert" arialabel="close">&times;</a><strong>
			...
			';
			exit;
		}if(filter_var($_POST['text2'],FILTER_VALIDATE_INT)){
			echo'
		<div class="alert alert-danger"> <a href="#" class="close" data-dismiss="alert" arialabel="close">&times;</a><strong>
...
echo'
		<div class="alert alert-danger"> <a href="#" class="close" data-dismiss="alert" arialabel="close">&times;</a><strong>
			...
			';		
				exit;
		}if(!empty($_POST['text9'])){
			if(filter_var($_POST['text9'],FILTER_VALIDATE_INT)){
				echo'
		<div class="alert alert-danger"> <a href="#" class="close" data-dismiss="alert" arialabel="close">&times;</a><strong>
			Alternative pickup Fullname cant contain numbers
</strong></div>
			';
			exit;
		}
	 ...
	 <form method="post" action="">
		<input type="submit" name="suborder" class="btn btn-info" style="width:100%;" value="Submit Order">
	 </form>';	 
	}
...
   				 }
	...
	if($user ==""){
	 ...
	 $sql9 = "SELECT * FROM `users` WHERE `id` = '$user' ";
		...
		$result = mysqli_num_rows($query7);
		if($result > 0){
				echo 'Order Has Already Been Submitted Successful!';
				exit();
		}else{
	 $sql2 = "INSERT INTO customerorder(Sellerid,`id`,`prod_id`,`user_id`,`generateid`,
	 `prod_title`,`prod_image`,`qty`,`price`,`totalpx`) SELECT Sellerid,`id`,`prod_id`,`user_id`,
	 `generateid`,`prod_title`,`prod_image`,`qty`,`price`,`totalpx` FROM cart where `generateid` = '$genrateId' ";
	 $query2 = mysqli_query($conn,$sql2);
	 if($query2){
		 $sql3 = "INSERT INTO `idtable` (`id`, `genrateId`) VALUES (NULL, '$genrateId')";
		 $query3 = mysqli_query($conn,$sql3);
...
switch($result){                                           
	...
	session_unset($_SESSION['Refids']);
	 ...
	 $sql2 = "INSERT INTO customerorder(Sellerid,`id`,`prod_id`,`user_id`,`generateid`,
	 `prod_title`,`prod_image`,`qty`,`price`,`totalpx`) SELECT Sellerid,`id`,`prod_id`,`user_id`,
	 `generateid`,`prod_title`,`prod_image`,`qty`,`price`,`totalpx` FROM cart where `user_id` = '$user' and orders = '0'";
	 $query2 = mysqli_query($conn,$sql2);
	 if($query2){
		 $sql3 = "UPDATE `cart` SET `orders` = '1' where `user_id` = '$user' ";
		 $query3 = mysqli_query($conn,$sql3);
...
switch($result){                                           
   ...
   if(isset($_POST['editProfile'])){
	...
	if(isset($_POST['GenReport2'])){
	 ...
	 if(isset($_POST['ediBok'])){
		...
		if($query){
			$row = mysqli_fetch_array($query);
			echo'
				<center style="font-weight:bold;font-family:Tahoma;">Edit BOOK</center>	
				<img class="pull-right"src="images/'.$row['prod_img'].'" height="100" width="100">
                                                     		<div class="col-md-12">
																	<label>Book ID</label>
																	<input type="text" class="form-control" id="buifd" value="'.$row['id'].'" name="buifd" placeholder="Book Title" required>
															</div>			
															<div class="col-md-12">
																	<label>Book Title</label>
																	<input type="text" class="form-control" id="lname" value="'.$row['prod_title'].'" name="lname" placeholder="Book Title" required>
															</div>
		
																<div class="col-md-12">
																<label>Select Book Category</label>
																	 <div class="form-group">
				                                                     <input type="text" class="form-control" value="'.$row['prod_cat'].'"  id="pwd2" name="pwd2" placeholder="Authour" required>    
																	</div>
															</div>									
															<div class="col-md-12">
																	<label>Book ISBN</label>
																	<input type="number" class="form-control" value="'.$row['prod_isbn'].'"  id="pwd" name="pwd" placeholder="Book ISBN" required>
															</div>
															<div class="col-md-12">
																	<label>Author Name</label>
																	<input type="text" class="form-control" value="'.$row['author'].'"  id="autho" name="autho" placeholder="Authour" required>
															</div>																									
															<div class="col-md-12">
																	<label>Price</label>
																	<input type="number" class="form-control" value="'.$row['prod_px'].'"  id="aln1" name="aln1" placeholder="Price" required>
															</div>
															<div class="col-md-12">
																	<label>Book Description</label>
																	<div class="form-group">
				                                     <textarea rows="9" class="form-control" id="drudpre" name="drudpre"placeholder="Book Description" style="resize:none;">
													 '.$row['prod_desc'].'
													 </textarea>
			                                                     </div>
															</div>																																																						
													<input style="margin-top:20px;"type="submit" id="updatebook" name="updatebook"  value="Update" class="btn btn-success btn-sm pull-right"/></div>
								
			';
			
		}
  }
	function generatidgaian(){
	$sql = "SELECT MAX(`genrateId`) as maximum_num FROM idtable ";
	 $query = mysqli_query($conn,$sql);
	 if($query){
	 $row = mysqli_fetch_array($query);
	...
	if(isset($_POST['unsetrefid'])){
		unset($_SESSION['Refids']);
		echo"unseted";
	}
