<?php
	...
	if(isset($_POST['getCateg'])){
		...
		if($query){
			...
			for($i=1; $i<=$pageno; $i++){
				$paga.="
			<li><a href='#' cid='$id' page='$i' id='Pages'>$i</a></li>
				";
			}
	...
	if(isset($_POST['getProds'])){
		$limit = 6;
		$prdctid = $_POST['cid'];
		if(isset($_POST['setPagess'])){
		    $page = $_POST['id'];
		    $pageStart = ($page * 6) - 6;
		}else{
			$pageStart = 0;
		}
		$sql = "SELECT * FROM `products` WHERE `prod_cat` = '$prdctid' limit $pageStart,$limit";
		$query = mysqli_query($conn,$sql);
		if($query){
			while($row = mysqli_fetch_array($query)){				;
				echo'
...
Item Has Already Been Added To Cart, Continue Shopping </div>';
				...
				$sql3 = "INSERT INTO `cart`(`prod_id`, `user_id`, `generateid`,`prod_title`, `prod_image`, `qty`, `price`, `totalpx`, date)
				VALUES ('$id','$user','".$_SESSION['genrateid']."','$prtitle','$prdimage','1','$prdpx','$prdpx',CURRENT_DATE) ";
