...
// toast('Success Toast','success');
  ...
  public function home(){
    ...
    // $event->name = '  Ogua Ahmed Event Check';
    ...
    //create an event based on a simple text string like this:
    // $event = new Event();
...
';
 ...
 public function edit_sales($id){
  ...
  return view('Sales.edit_sales',['sale'=>$sales]);
 }

public function edit_sales_data(Request $request, $id){
   ...
   //get sales
     ...
     //fetch products
     $product = Product::where('id',$prductid)->first();
     $prqty = $product->quantuty;
     ...
     if ($type == 'Bundle Product') {
        ...
        //now check if a bundle was sold or a sinlge item was sold
      if ($sales->sold == 'Sold out of a Bundle') {
           #note a sinlge must be add to the singlecheck
           #fetch product Single Check

          $single = Singlecheck::where('product_id',$product->id)->first();
          $qtyleft = $single->qytleft;
          $qtysold = $qty;

          //check the sold from the incoming quantity 
          if ($qty > $request->input('prodcut-qty')) {
            //there is a decrement in a single product
            $sinlet = $qty - $request->input('prodcut-qty');
            //add to single check product
            $single->qytleft = $qtyleft + $sinlet;

          }elseif ($qty < $request->input('prodcut-qty')){
             $sinlet =$request->input('prodcut-qty') - $qty;
            //there is a increment in a single product
            $single->qytleft = $qtyleft - $sinlet;
          }

          $single->save();

          //dd("this is a single product");
      }else{
          //a bundle was sold
        if ($qty > $request->input('prodcut-qty')) {
        //there is been a decrement in the quantity of the product.
         $qyydiff = $qty - $request->input('prodcut-qty');

       //add to quantity before sale.
         $sales->nbs = $qtybfs + $qyydiff;

       //add to quantity after sale
         $sales->nas = $qtyafs + $qyydiff;

       //add the decrement to the product quantity
         $product->quantuty = $prqty + $qyydiff;

         $sales->save();
         $product->save();         

       }elseif ($qty < $request->input('prodcut-qty')){
         //there is been a increment in the quantity of the product.
         $qyydiff = $request->input('prodcut-qty') - $qty;

         //substract from quantity before sale.
         $sales->nbs = $qtybfs + $qyydiff;

         //substract from quantity after sale
         $sales->nas = $qtyafs - $qyydiff;

         //substrct from products
         $product->quantuty = $prqty - $qyydiff;
       ...
       //dd("this is a Bundle product");

     }
   }else{

       //this is a single product
       #not from a bundle or a bundle
        if ($qty > $request->input('prodcut-qty')) {
        //there is been a decrement in the quantity of the product.
         $qyydiff = $qty - $request->input('prodcut-qty');
 ...
 $sales->quantity = $request->input('prodcut-qty');
 $sales->price = $request->input('product-price');
 $sales->type = $request->input('product-type');
 $sales->save();

 return Redirect()->back()->with('message', 'Sales Editted Successfully');
...
public function sales_per_month_year($mnth,$year,$code){


  $cchck = User::where('id',auth()->user()->id)->first();  
  $cc = $cchck->hasRole('worker');
  if ($cc) {
    $sales = Sale::whereMonth('created_at',$mnth)
...
public function change_password(){
  return view('UserRoles.change_password');
}
...
public function change_user_password(Request $request){
  ...
  $credentials = [
    'email' => $email,
    'password' => $request->input('prvpass')
  ];  

   if (Auth::attempt($credentials)) {
        $new = $request->input('password');
        $confrm = $request->input('password_confirmation'); 

        //check password length

        if (strlen($new) < 6) {
          return Redirect()->back()->
          withInput($request->all())->with('messages','Password Must Contain Atleast 8 Characters!');
        }
        ...
        if ($new == $confrm) {
          ...
          return Redirect()->back()
          ->
          withInput($request->all())->with('messages','Password Do Not Match!');
        } 

   }else{
    return Redirect()->back()->
          withInput($request->all())->with('messages','Password Is Incorrect!');
   }
...
public function ussd(Request $request){
        ...
        */
        ...
        if ($text == "") {
          $response = "Con Welcome to OguSesTSolutons payment platform";
          $response .= "1. Make Payement";
          $response .= "2. Enter User Details";
        }elseif ($text == '1') {
          $response = "Con Enter your phone number";
        }elseif ($text == '2') {
          $response = "End Your name is Ahmed Ahia";
        }
