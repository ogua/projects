...
<div class="header2 text-center">
	 <div class="container">
		...
		<ul class="megamenu skyblue">
			...
			<li class="active grid"><a class="color1" href="vieworders.php">Delivery Status</a></li>
...
<div class="cart">
	 <div class="container">
		...
		<?php
				include_once('db.php');
	    $sql = "SELECT * FROM `customerorder` WHERE `Sellerid` = '".$_SESSION['sellid']."' AND Status = '2' group by ReFid";
				...
				$prod_id = $row['ReFid']; 
				 ...
				 <div class="cart-sec">
					   <div class="cart-item-info">
							 ...
							 ';
								if($row['Status'] == 4){
								echo '<a href="#" id="curntstst" class="btn btn-info btn-sm pull-right" >Deliveried</a>';
							}elseif($row['Status'] == 2){
							...
							<select class="form-control pull-right form-control-sm UpSta" cid='.$row['ReFid'].' id="UpSta" name="UpSta">
							  <option value="3">Delivery Cancelled</option>
		...
		<?php
			$sq = "SELECT * FROM `customerorder` WHERE `Sellerid` = '".$_SESSION['sellid']."' AND Status = '4' group by ReFid";
		    $quer = mysqli_query($conn,$sq);
			?>
			<table id="members" class="backimage table table-bordered">
						<tr>
							<td colspan="2"><h2><b><center>Delivery Details</center></b></h2></td>
						</tr>
						<tr>
			<?php
			while($rows = mysqli_fetch_array($quer)){
				...
				<td>Fullname</td>
						...
						<tr>
							<td>Status</td>
							<td><?php
									echo '<a href="#" id="curntstst" class="btn btn-info btn-sm" >Deliveried</a>';
							?></td>
						</tr>
						<tr>
							<td colspan="2"><hr style="border:2px solid #fff;"></td>
						</tr>
